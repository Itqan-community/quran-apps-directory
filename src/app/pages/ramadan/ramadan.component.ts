...
export class RamadanComponent implements OnInit, OnDestroy {
  ...
  private readonly sectionDefinitions: Omit<RamadanSection, 'apps'>[] = [
    {
      ...
      description_ar: 'يحظى القرآن في رمضان بحضور يومي منتظم في حياتنا، تتنوع معه أنماط القراءة والاستماع والتدبر.\nتضم هذه المجموعة تطبيقات تساعدك على قراءة القرآن، الاستماع إليه، تدبره، ومراجعته بأساليب مختلفة تناسب إيقاع الشهر.',
      title_en: 'Quran in Ramadan',
      subtitle_en: 'Daily companion for recitation, reflection and memorization',
      description_en: 'The Quran has a special daily presence in our lives during Ramadan, with diverse modes of reading, listening, and reflection.\nThis collection includes apps to help you read, listen to, reflect upon, and review the Quran in ways that suit the rhythm of the month.',
  ...
  ngOnInit(): void {
    this.sections = this.sectionDefinitions.map((def) => ({ ...def, apps: [] }));
  ...
  getSectionSubtitle(section: RamadanSection): string {
    return this.currentLang === 'ar' ? section.subtitle_ar : section.subtitle_en;
  }

  getSectionDescription(section: RamadanSection): string {
    return this.currentLang === 'ar' ? section.description_ar : section.description_en;
  }
